...
const register = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (password.length < 6) return next(new ApiError("Password must be at least 6 characters", 400));
    ...
    res
      .cookie("__access_token", accessToken, { ...cookieOptions, expires: accessExpires })
...
const uploadAvatar = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { user } = req.body;
    const avatarFilePath = (req.files as { [fieldname: string]: Express.Multer.File[] }).avatar?.[0]?.path;

    if (!avatarFilePath) return next(new ApiError("Please provide an image", 400));

    const avatarUploadResponse = await cloudinary.uploader.upload(avatarFilePath, {
    ...
    user.avatar = avatarUploadResponse.secure_url;
    await user.save();

    res.status(200).json(new ApiResponse("Avatar uploaded", 200, { user: user.toJSON() }));
  } catch (error: any) {
...
const checkEmail = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(200).json(new ApiResponse("Email checked", 200, { isExist: !!existingEmail }));
  } catch (error: any) {
    console.error("Error in Check Email :: ", error?.message);
    return next(new ApiError(error?.message));
  }
};
